...
void test_pid_init() {
    ...
    EXPECT_DOUBLE_EQ(pid.config.kp, 1.0);
}
...
void test_pid_update() {
    ...
    EXPECT_TRUE(isfinite(output)); /* pid_update produces a finite output */
}
...
void test_pid_types() {
    ...
    /* pid_config_t type compiles and is instantiable */

    pid_state_t state;
    memset(&state, 0, sizeof(state));
    /* pid_state_t type compiles and is instantiable */
